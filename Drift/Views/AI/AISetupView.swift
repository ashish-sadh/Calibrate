...
struct AIView: View {
    ...
    ///
    /// `.foundationModels` is system-managed (no weights to download, no
    /// per-call cost, ships with the OS), so when a user has explicitly
    /// selected it we go straight into the chat. Post-#872 NO-GO it is an
    /// opt-in, NOT the default: the default is on-device `.llamaCpp`/Gemma,
    /// which returns false here and routes through the chooser/download flow
    /// until the model is installed. Remote BYOK still requires a stored key.
