...
The thesis of this post, after running the loop for a year: in autonomous coding with language models, **human attention is the scarce resource, and the harness is where your taste actually lives.** Not in the model. Not in the prompt. In the scaffolding you build around the model — the gates that refuse bad tool calls, the reconciliation that won't trust session memory, the dashboards you can read from your phone, the personas that accumulate taste across hundreds of reviews.

The harness isn't about autonomy. It's about *choosing where to spend attention*.

I believe this because I've been running one. The loop is a bash `while true` around Claude Code — what Geoffrey Huntley calls a [Ralph loop](https://ghuntley.com/ralph/) — and over the last year it has built almost all of an iOS health-tracking app called **Drift**. Twenty-five friends and friends-of-friends now have it on their phones. In the seven days before I sat down to write this, the loop pushed **409 commits** to the repo, closed **30 bug issues** (most from real people on TestFlight, average close time around eleven minutes), and shipped **three TestFlight builds**. I wrote none of that code.

Two things live side by side in this post:

- **Drift** — the iOS app. The dish. The public beta is [on TestFlight](https://testflight.apple.com/join/NDxkRwRq) if you'd like to try it.
...
One more design choice: **no-"server" architecture.** No accounts. No subscription. No backend on my side. Even photo meal logging goes directly from your phone to your own API key — the only payload on the wire is the photo.

That's the app. The rest of this essay is what builds it.
...
The review ends with *Decisions for Human* — three numbered questions pinned to me. I read them in bed, tap approve on one, reply "defer" on another. The next planning cycle picks them up. That is the shape of spending attention deliberately: the personas converge on their own, and only the irreducible decisions reach me.
...
*Caveat: two samples from the same model aren't actually independent votes. The real next lever is letting beta-user A/B reactions become the eval — that piece is still half-built.*
