...
- Analytical tools category needs 3-5 live tools before the "AI health coach" identity holds. We're at 1 (cross_domain_insight). This cycle adds weight_trend_prediction (#334) and exercise_volume_summary (#335) — at 3 tools, we can begin marketing the angle. The tools are conceptually simple (linear regression on existing service data) but high perceived value. Ship fast, don't over-engineer.
- 'am I hitting my protein goal' has been in failing-queries.md for 2+ cycles. The fix is a data model change (macro targets in UserPreferences) that also unlocks a whole goal-tracking feature. This is the kind of fix that pays compound dividends — fix the data model, get 4+ failing queries resolved at once. Prioritize structural fixes over one-off prompt tweaks.
- Chat help card ('what can you do?') is the best single discoverability investment after Settings → Feedback. Users who don't know what to ask don't file feature requests — they just stop opening the app. A one-tap overview of capabilities with tappable examples converts curious users to power users. Every AI-first app needs this; we don't have it yet.
- Suggestion pills gated on data existence is the right UX pattern for analytical features. Showing 'Do I spike after meals?' to a user with 2 food entries is confusing; showing it to a user with 30 days of glucose + food data is delightful. Gate on data, not on feature flag.
