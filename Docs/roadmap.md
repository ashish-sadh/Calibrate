...
- **AI Chat depth — eval quality, tie-break & analytical expansion (P0, cycle 4949 sprint)** — State.md refresh to build 167 (#394). FoodLoggingGoldSet run + fix failures — mandatory eval cycle (#395). IntentClassifier tie-break: route close-scored queries to clarification (gap < 0.15) (#396). Food DB +30 Middle Eastern cuisine (#397). Food DB +30 Mexican home cooking (#398). Telemetry-driven Stage 1/3 prompt refresh — cluster top failures, add 3+ few-shot examples (#399). MultiTurnRegression +5 hydration + multi-intent correction chains (#400). failing-queries.md refresh cycle 4910→4949 (#401). `weight_trend_prediction` analytical tool: "when will I reach my goal?" via linear regression on weight log (#402). Queue cap of 70 sprint tasks enforced — no new tasks until queue drops below 70.
...
### Now
- **`weight_trend_prediction` analytical tool (P1, #402)** — "When will I reach my goal weight?" — linear regression on weight log, projected date + weekly rate + confidence.
