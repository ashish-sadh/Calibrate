...
- **AI Chat depth — analytical tools expansion & failing-query closure (P0, cycle 4521 sprint)** — `weight_trend_prediction` analytical tool: "when will I reach my goal?" (#334). `exercise_volume_summary` analytical tool: "how's my training volume?" per muscle group (#335). Macro goal targets in UserPreferences — fixes 'am I hitting my protein goal' failing query (#336). Chat help card — 'what can you do?' → structured capability overview with tappable examples (#337). FoodLoggingGoldSet +10 analytical query cases (#338). MultiTurnRegression +10 analytical follow-up chains (#339). Food DB +30 Middle Eastern/Persian (#340). state.md accuracy refresh (#341). Suggestion pill bar analytical examples gated on data existence (#342).
