...
## Numbers
- **Version:** 0.1.0, Build 382 (iOS) · 101 (Android Play internal)
- **Tests:** 1,437 iOS DriftTests (1,239 swift-testing + 198 XCTest; UI/HealthKit/OCR-bound only — bulk of suite migrated down) + 2,781 DriftCoreTests in 150 suites (cross-platform pure-logic suite, 1 known issue); LLM eval ~160+ cases in DriftLLMEvalMacOS. (Both counts re-measured 2026-08-18.)
...
- **Exercises:** 960 (free-exercise-db) + 126 registered customs (`DefaultTemplates.customExercises`, registered at every launch, idempotent — the Drift Package template vocabulary + gym vernacular). Name resolution: `ExerciseDatabase.match` = exact → `ExerciseAliases` phrase table → full token coverage → **longest catalog name contained in the phrase** (the 2026-07-29 fallback: full coverage alone found 1 of a real user's 10 logged exercises, because one unknown qualifier — "hold", "box", "knee" — sank the whole line).
...
- **AI Chat Features:** 25+ (see `Docs/ai-parity.md`); Coach interview "set me up" → TrainingProfile → Qwen-generated weekly routine (equipment-filtered, grounded, offline fallback); "log my usual push day" replay; tappable quick-reply chips; ActiveWorkout command strip ("add face pulls" / "drop curls" / "last bench?"); Workout scan (builds 358-359) — photo/screenshot/PDF of any workout page → Nebius Qwen2.5-VL reads it (handwriting incl.) and auto-splits into reusable template(s) + dated logged session(s) with per-set weights, review-before-save (`NebiusWorkoutPhotoParser`, `WorkoutScanSheet`); optional user note travels with the scan as authoritative context
