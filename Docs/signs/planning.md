...
- **The stuck-epic override is count+counter-driven, NOT updatedAt-driven — so a genuinely human-blocked epic re-fires override every ~3 planning-due cycles no matter what you label it or how you bump its timestamp.** The trigger (`self-improve-watchdog.sh:849-874`) is `_count_open_epics() ≥1` (line 134 counts every open `epic`-labeled issue, with no `needs-human`/`blocked` exemption) plus a `stuck-epic-cycles` counter that overrides on the 3rd planning-due cycle. The only honest stops are a human clearing the blocker or closing the epic; bumping `updatedAt` and adding `needs-human` are both no-ops for this path. When the re-fire is a real infra block (not queue latency), confirm the blocker still holds, route to the human via the exec briefing, and exit cheaply — do NOT re-scope, and do NOT expect a label to silence the watchdog.
  Why: #909 (4th override, cycle 20840, 2026-06-30) — prior reaffirms (lines 33/42) blamed the epic's `updatedAt`; reading the watchdog showed the stuck-epic path never reads updatedAt, so the override is structurally guaranteed to re-fire on any long-lived open epic. #909's re-fire was correctly routed to the human (#913 sim outage, still down on 06-30) rather than treated as a scoping signal.
