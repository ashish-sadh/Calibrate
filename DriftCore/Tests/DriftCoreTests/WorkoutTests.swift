...
// MARK: - Mid-workout note edits write back to the template (#1169)
//
// The operator's complaint: notes are pre-filled FROM the template, so editing
// one there is the obvious place to fix a cue — but the edit died with the
// session and the old note was back tomorrow.

/// The plain case: a cue edited during the workout sticks to the template.
@Test func editedNoteFoldsBackIntoTemplate() {
    let existing = [
        WorkoutTemplate.TemplateExercise(name: "Bench Press", sets: 3, notes: "6-8 reps"),
        WorkoutTemplate.TemplateExercise(name: "Row", sets: 3, notes: "10 reps"),
    ]
    let merged = WorkoutService.templateExercisesApplyingNoteEdits(
        existing,
        edited: ["Bench Press": "6-8 reps, elbows tucked", "Row": "10 reps"],
        baseline: ["Bench Press": "6-8 reps", "Row": "10 reps"])
    #expect(merged?[0].notes == "6-8 reps, elbows tucked")
    #expect(merged?[1].notes == "10 reps", "an untouched exercise keeps its note")
}

/// THE GUARD. `addExercise` auto-fills "Tip: …" for any exercise the template
/// left blank. Those are generated, not typed — writing them back would quietly
/// author the user's template for them.
@Test func autoGeneratedTipIsNeverWrittenIntoTheTemplate() {
    let existing = [WorkoutTemplate.TemplateExercise(name: "Squat", sets: 5, notes: nil)]
    // Baseline == what the live workout showed on open, tip included.
    let merged = WorkoutService.templateExercisesApplyingNoteEdits(
        existing,
        edited: ["Squat": "Tip: keep your chest up"],
        baseline: ["Squat": "Tip: keep your chest up"])
    #expect(merged == nil, "an untouched auto-tip is not an edit")
}

/// Nothing changed → no write at all, so the 30s persist tick stays free.
@Test func unchangedNotesProduceNoTemplateWrite() {
    let existing = [WorkoutTemplate.TemplateExercise(name: "Deadlift", sets: 3, notes: "5x3")]
    #expect(WorkoutService.templateExercisesApplyingNoteEdits(
        existing, edited: ["Deadlift": "5x3"], baseline: ["Deadlift": "5x3"]) == nil)
}

/// Everything that is not a note survives untouched — adding a set today is
/// adapting the session, not redefining the plan.
@Test func noteWriteBackLeavesEveryOtherTemplateFieldAlone() {
    let existing = [
        WorkoutTemplate.TemplateExercise(name: "Plank", sets: 4, isWarmup: true,
                                         restSeconds: 45, notes: "30s", isDuration: true),
    ]
    let merged = WorkoutService.templateExercisesApplyingNoteEdits(
        existing, edited: ["Plank": "45s"], baseline: ["Plank": "30s"])
    #expect(merged?[0].notes == "45s")
    #expect(merged?[0].sets == 4)
    #expect(merged?[0].isWarmup == true)
    #expect(merged?[0].restSeconds == 45)
    #expect(merged?[0].isDuration == true)
}

/// Clearing a note stores nil, not "" — the template load path treats nil as
/// "no note" and would otherwise render an empty cue line forever.
@Test func clearingANoteStoresNilNotEmptyString() {
    let existing = [WorkoutTemplate.TemplateExercise(name: "Curl", sets: 3, notes: "slow")]
    let merged = WorkoutService.templateExercisesApplyingNoteEdits(
        existing, edited: ["Curl": "   "], baseline: ["Curl": "slow"])
    #expect(merged?[0].notes == nil)
}

/// An exercise added mid-workout has no template row to write to, and must not
/// invent one — the template's shape is only ever changed deliberately.
@Test func exerciseAddedMidWorkoutDoesNotJoinTheTemplate() {
    let existing = [WorkoutTemplate.TemplateExercise(name: "Bench Press", sets: 3, notes: "6-8")]
    let merged = WorkoutService.templateExercisesApplyingNoteEdits(
        existing,
        edited: ["Bench Press": "6-8", "Cable Fly": "burnout"],
        baseline: ["Bench Press": "6-8"])
    #expect(merged == nil, "a new exercise is not a note edit")
}
