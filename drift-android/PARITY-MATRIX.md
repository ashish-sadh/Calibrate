...
| Chrome | 180s auto-refresh poll — Android reloads on `.foodEntryAdded` + onAppear instead. Still a substitution, but the onAppear half is throttled now (#1202, `4e5a`): Android has no background poll, so data written by a path that does NOT force (chat-logged weight, Health Connect sync) can sit up to 30s stale on a tab return. | deviation | #1202 |
