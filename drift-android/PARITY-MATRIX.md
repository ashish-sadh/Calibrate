...
| screen | sub-interaction | status | issue |
|---|---|---|---|
| Food tab root | date strip — renders Sat 15→Sat 22 w/ Tue 18 pill. **Taps DO NOT change the day (scout #29, build 106, overturns scout #20's "taps change day")**: tapping `Sun 16` at the a11y-derived centre of its `clickable="true"` wrapper leaves the pill, the macro card and the diary on today at +3s (an *immediate* capture is NOT evidence here — `screencap` samples near the start of the call, see [[skip_fuse_compose_recomposition_delivery]]). Android also windows the strip to −3/+7 days vs iOS −30/+7 (`FoodTabView.swift:385-397`, deliberate: `ScrollViewReader.scrollTo` never fires on Fuse) — so with the month sheet also inert, **no day older than 3 days is reachable at all** | broken | **#1254** (P0) |
...
| Food tab root | Select Date — **day taps are INERT**: tapping `Tue, Aug 4, food logged` (the only day in the DB with entries) or `Aug 11` or `Jul 15` dismisses the sheet and leaves the tab on today (month label stays "August 2026" even for the July pick); re-opening still highlights today. The `onSelect` closure demonstrably runs (its 3rd statement `showingDatePicker = false` closes the sheet) while its 1st, `viewModel.goToDate(date)`, does not take — and `shiftMonth`'s local `@State` write in the same sheet works | broken | **#1254** (P0) |
...
| Weight tab | `1W` chip re-windows the chart instead of deleting the card — **FIXED, verified**: 1W → `Aug 16…Aug 18`, `Average 163.6`, `Difference −1.0 lbs`, chart card intact *(driven #31, build 111; row was `broken` against CLOSED #1220 for a week)* | ok | — |
...
| Weight tab | chart X-axis is windowed to the **first data point**, not to the range start — 1W labels `Aug 16` where iOS's `rangeStart` would label `Aug 11`, so a sparse week renders compressed *(observed #31; iOS `WeightChartView(rangeStart:)`)* | deviation | #1205 |
...
| Weight tab | stats header — **self-contradicting, now in four places**: `164.2 lbs` over `Current: 174.2lbs` (10 lb apart, both labelled "current"); `7-day change: -6.1lbs` beside a `7d +1.3 lbs` chip (**opposite signs**); `30-day change: -7.3lbs` beside `30d -16.1 lbs`. Worse, `changeChip` (`WeightTab.swift:551-556`) hardcodes `down = green` and ignores `store.isLosing`, so the **same `+1.3 lbs` is RED in the chip and GREEN in the history row on the same screen**. Executor's in-flight tree deletes the whole card *(driven #31)* | broken | #1205 |
...
| Weight tab | nav title renders LARGE; iOS pins `.navigationBarTitleDisplayMode(.inline)` — Android now sets `.toolbar(.hidden)` instead, so there is no title at all | deviation | #1200 |
...
| Weight tab | WeightInsightsView: CURRENT/WEEKLY + EST. BALANCE/PROJECTED stat cards, `Trend Weight` pill, 3/7/14/30/90-day change table with sparklines, Body Composition + Add CTA, weekday-pattern line — **absent at build 111; executor's tree moves `WeightInsightsView.swift` into `SharedUI/`, so this is landing now** *(iOS reference captured #31)* | missing | #1205 |
